#ifndef _MAIN_WND_FRAME_
#define _MAIN_WND_FRAME_
...
class CMainWndFrame : public WindowImplBase, public CDwm , public CDPI {
protected:
	...
	bool OnListEvent(void* wparam);

protected:
	void OnExitBtn(TNotifyUI& msg);
	void OnLoginBtn(TNotifyUI& msg);
	void OnConnectBtn(TNotifyUI& msg);
	void OnOptionBtn(TNotifyUI& msg);
...
private:
	...
	void switchPlane(plane_t plane);

private:
	CVerticalLayoutUI *m_pPorxyList = NULL;
	void AddPorxyItem(int state, std::string ip, std::string country,std::string city);
	bool OnItemClick(void* wparam);
